...
static int send_screen_info_msg(daemon_ctx *ctx, int fd) {
  struct ctrl_msg hdr = {.type = CTRL_MSG_SCREEN_INFO,
...
static void try_deliver_fds(daemon_ctx *ctx) {
  ...
  if (send_fds(ctx->producer->ctrl_fd, &msg, sizeof(msg), ctx->deposited_fds,
               ctx->deposited_fd_count) < 0) {
...
/*
 * Tear down whichever role this client holds, reset the associated state, then
 * free it. Used both for real disconnects (EPOLLHUP / recv error) and to evict
 * a stale client when a new one takes over the same role, whether the old
 * client is still alive or already a ghost, finding one is reason enough to
 * drop it.
 *
 * The role pointer is cleared BEFORE client_free() so that any event still
 * queued for this client in the current epoll batch fails the "is it a current
 * role?" guard in the main loop and is skipped instead of dereferencing freed
 * memory.
 */
static void drop_client(daemon_ctx *ctx, struct client *c) {
  ...
  if (c == ctx->consumer) {
    ...
    ctx->consumer = NULL;
    /* The deposited fds belong to the consumer that just left; a future
     * producer must never be handed this stale set. Drop them together with the
     * consumer. */
...
static void handle_client_data(daemon_ctx *ctx, struct client *c) {
  ...
  if (hdr.size > 0) {
    if (hdr.size > sizeof(payload) ||
        recv_all(c->ctrl_fd, payload, hdr.size) < 0) {
      drop_client(ctx, c);
      return;
    }
  }
...
static void handle_new_connection(daemon_ctx *ctx, int listen_fd) {
  ...
  if (hdr.type == CTRL_MSG_CONSUMER_HELLO) {
    /* Evict any prior consumer (alive or ghost) and its stale deposit before
     * this one takes over the role. */
    if (ctx->consumer)
